...
if (req.params.id == 'current') {
	redis.lrange('runs', 0, 0, function (err, data) {
		if (err) {
			console.log('Redis error:', err);
			return;
		}
		var run = JSON.parse(data[0]);
                var lastrun = run['run_id'];
	        redis.lrange(lastrun, 0, -1, function (err, data2) {
		    if (err) {
			console.log('Redis error:', err);
			return;
	         	}
		    ...
		    for (var i=0; i<data2.length; i++) {
		     	payload.push(JSON.parse(data2[i]));
	     	}
